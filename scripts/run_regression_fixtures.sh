...
expect_pass() {
  local name="$1"
  shift
  local out="$TMP_OUT_DIR/${name}.out"
  if "$@" >"$out" 2>&1; then
...
expect_pass issue_map_good python3 scripts/check_reviewer_issue_map.py examples/issue_map_response_good.md --map schemas/reviewer_issue_map.example.csv --reviewers R1,R2,R3,R4 --fail-on P1
...
expect_pass response_good python3 scripts/check_response_text.py examples/platform_response_good.md --reviewers R1,R2,R3,R4 --require-reviewers --platform openreview --max-words 220 --max-chars 1500 --fail-on P0
expect_pass promises_good python3 scripts/check_revision_promises.py examples/revision_promises_good.md --fail-on P1
expect_pass dashboard_views python3 scripts/render_dashboard_views.py
...
expect_fail issue_map_bad python3 scripts/check_reviewer_issue_map.py examples/issue_map_response_good.md --map examples/regression_bad/bad_reviewer_issue_map.csv --reviewers R1,R2,R3,R4 --fail-on P1
...
expect_fail response_bad python3 scripts/check_response_text.py examples/regression_bad/bad_platform_response.md --reviewers R1,R2,R3,R4 --require-reviewers --platform openreview --max-words 220 --max-chars 1500 --fail-on P0
